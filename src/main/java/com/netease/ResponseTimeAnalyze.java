...
public class ResponseTimeAnalyze {
    public static void main(String args[]) throws IOException, ParseException {
            ...
            if(line.contains("method = ")&&line.contains("cost_time=")){
                ...
                if(matcher_method.find()&&matcher_costTime.find()){
                   // mailMap.put("date",line.substring(7,30));
                    mailMap.put(matcher_method.group().substring(9,matcher_method.group().length()-1),
                            matcher_costTime.group().substring(10,matcher_costTime.group().length()));
                    //mailMap.put("costTime",);
        ...
        for(Object tmp:uniqueMethod){
            mapcoll = mailMap.getCollection(tmp);
            Object[] TM = mapcoll.toArray();
            int sum =0;
            int ave = 0;
            for(int i=0;i<TM.length;i++){
                sum += Integer.parseInt(TM[i].toString());
            }
            ave  = sum/TM.length;
            writer.write(tmp+":"+ave);
            writer.write("\r\n");
